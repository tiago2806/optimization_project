...
def pso(generate_solution, fitness_function,
    ...
    #Each particle remembers its own best position... at the start, each particle's best position is the initial position
    personal_best_positions = [particles[i][:] for i in range(n_particles)]  #a função de [:] é muito importante!! 
    personal_best_fits = [fits[i] for i in range(n_particles)]
 
    #The swarm remembers the single best position found by any particle
    global_best_idx = np.argmax(personal_best_fits) #returns the index of the particle with best fitness
    global_best_position = personal_best_positions[global_best_idx][:] #Copy of the position that has the best personal_best
    global_best_fitness = personal_best_fits[global_best_idx] #Stores the fitness score of that best particle
    ...
    for iteration in range(n_iterations):
        ...
        for i in range(n_particles):
            ...
            #Update position: x = x + v, i.e., new position = old position + new velocity... note: a position is the weight vector of the particle
            new_position = []
            for j in range(n_weights):
